...
export const Axios_Get_Moduls = async (Path_URL, Params_Data) => {
  try {
    ...
    const Axios_Get_Moduls_Axios = await axios.get(
      `${process.env.REACT_APP_DB_HOST}${Path_URL}`,
      {
        params: Params_Data,
        headers: {
          Authorization: token,
...
export const Axios_Post_Moduls = async (Path_URL, Post_Data) => {
  try {
    const token = localStorage.getItem("Login_token");
    const id = localStorage.getItem("Login_id");
    const Axios_Post_Moduls_Axios = await axios.post(
      `${process.env.REACT_APP_DB_HOST}${Path_URL}`,
      Post_Data,
      {
        headers: {
  ...
  try {
    ...
    if (Post_Axios.status === 200) {
      ...
      if (Post_Axios.data.status === 200) {
        ...
        Post_Axios.data.status === 600 ||
        Post_Axios.data.status === 401
      ) {
