...
const CommentWrite = ({ list, All_Comment_List, setAll_Comment_List }) => {
    ...
    const Handle_Click_Save_New_Comment = async () => {
        try {
            ...
            const Handle_Click_Save_New_Comment_Axios = await Axios_Post_Moduls('/LocalPim/Handle_Click_Save_New_Comment', {
                Select_Data: list,
                Comment_Write_State,
            ...
            if (Handle_Click_Save_New_Comment_Axios) {
    ...
    const Insert_Data_Func_Cal = (All_Data, Random_key_Comment, InsertData) => {
        return All_Data.map(item => {
            if (
                item.kizuki_notepad_kizuki_comment_code ===
                (list.kizuki_notepad_kizuki_comment_parent_code === 'TOP'
                    ? list.kizuki_notepad_kizuki_comment_code
                    : list.kizuki_notepad_kizuki_comment_parent_code)
            ) {
                return {
                    ...item,
    ...
    const Insert_Data = async Random_key_Comment => {
        const InsertData = {
            ...
            kizuki_notepad_kizuki_comment_parent_code:
                list.kizuki_notepad_kizuki_comment_parent_code === 'TOP'
                    ? list.kizuki_notepad_kizuki_comment_code
                    : list.kizuki_notepad_kizuki_comment_parent_code,
    ...
    return (
        <CommentWriteMainDivBox>
            <input
                type="text"
                value={Comment_Write_State}
                onChange={e => setComment_Write_State(e.target.value)}
                placeholder="댓글을 작성 바랍니다."
            ></input>
