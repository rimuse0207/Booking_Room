...
import ScheduleRegistration, { DivideType } from './ScheduleRegistration/ScheduleRegistration';
...
const DayCell = styled.div`
    min-height: ${({ isHeader }) => (isHeader ? '30px' : '140px')};
    padding: 5px;
...
const Calendar = ({ year, month, onMonthChange, events, Getting_pims_Lists_From_Date }) => {
    ...
    const eventBars = events.flatMap(event => {
        ...
        while (currentStart <= endIndex) {
            ...
            bars.push({
                ...event,
                top: (Math.floor(currentStart / 7) + 1) * 140,
    ...
    const handleMouseEnter = (e, event, index) => {
        ...
        let popupY = e.clientY - parentRect.top + parentElement.scrollTop - 70; // 부모 요소 내 Y 좌표
        ...
        <div>
            <CalendarWrapper className="AnnualLeaveCalendarTableMainDivBox">
                ...
                <CalendarGrid>
                    ...
                    {eventBars.map((bar, idx) => (
                        ...
                        >
                            {DivideType.find(item => item.title === bar.title).name}
                        </EventBar>
                    ))}
                    {hoveredEvent && (
                        <div
                        ...
                        >
                            {/* <h2 style={{}}>{hoveredEvent.Code}</h2> */}
                            <strong style={{ fontSize: '1.2em' }}>
                                {' '}
                                {DivideType.find(item => item.title === hoveredEvent.title).name}
                                {/* {hoveredEvent.title} */}
                            </strong>
                            <div style={{ padding: '10px' }}></div>
                            <div>
                                <strong style={{ marginRight: '40px' }}>일자</strong>
                                <span>
            ...
            {ScheduleRegistrationIsModalOpen ? (
                <ScheduleRegistration
                    Choose_Start_Date={ChooseDate}
                    Choose_End_Date={ChooseDate}
