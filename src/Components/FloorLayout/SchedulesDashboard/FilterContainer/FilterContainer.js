...
const FilterContainer = ({
  ...
  const SelectedData = (selectMenu, selectData) => {
    const Select_Object = Selected_Lists.find(
      (list) => list.Type === selectMenu
    );

    let newSelectedLists;
    if (Select_Object.lists.includes(selectData)) {
      // 데이터 제거
      const Delete_Lists = Select_Object.lists.filter(
        (item) => item !== selectData
      );
      newSelectedLists = Selected_Lists.map((item) =>
        item.Type === selectMenu
          ? { ...item, lists: Delete_Lists }
          : { ...item }
      );
    } else {
      // 데이터 추가
      newSelectedLists = Selected_Lists.map((item) =>
        item.Type === selectMenu
          ? { ...item, lists: [...item.lists, selectData] }
          : { ...item }
      );
    }
    ...
    const cleaned = newSelectedLists.map((group) => {
      if (group.Type === "department") {
        const selectedPlace = newSelectedLists.find(
          (i) => i.Type === "place"
        ).lists;
        const before = group.lists;
        const after = before.filter((depValue) => {
          const depObj = Lists.find(
            (item) => item.Type === "department"
          ).lists.find((d) => d.value === depValue);
          return !selectedPlace.some((place) =>
            depObj.placeMatch.includes(place)
          );
        });
        const removed = before.filter((x) => !after.includes(x));
        if (removed.length > 0) {
          toast.show({
            title: `부서 선택값 [${removed.join(
              ", "
            )}] 이(가) 조건에 의해 해제되었습니다..`,
            successCheck: false,
            duration: 6000,
      ...
      if (group.Type === "team") {
        ...
        const selectedDep = newSelectedLists.find(
          (i) => i.Type === "department"
        ).lists;
        const before = group.lists;
        const after = before.filter((teamValue) => {
          const teamObj = Lists.find((item) => item.Type === "team").lists.find(
            (t) => t.value === teamValue
          );
          const blockedByDep = selectedDep.some((dep) =>
            teamObj.departmentMatch.includes(dep)
          );
          const blockedByPlace = selectedPlace.some((place) =>
            teamObj.placeMatch.includes(place)
          );
    ...
    <FilterContainerMainDivBox>
      <div>
            ...
            <li>
              ...
              >
                ...
                <div
                  ...
                  onClick={() =>
                    setSelected_Date(
                      moment(Selected_Date).add(1, "day").format("YYYY-MM-DD")
                    )
                  }
                >
                  <MdOutlineArrowForwardIos />
        ...
        <div className="Filter_Lists_Container">
          ...
          <ul>
            {Lists.find((item) => item.Type === "department").lists.map(
              (list) => {
                return Selected_Lists.find(
                  (item) => item.Type === "place"
                ).lists.some((matching) =>
          ...
          <ul>
            {Lists.find((item) => item.Type === "team").lists.map((list) => {
              return Selected_Lists.find(
                ...
                Selected_Lists.find((item) => item.Type === "place").lists.some(
                  (matching) => list.placeMatch.includes(matching)
                ) ? (
        ...
        <div className="Filter_Lists_Container">
          ...
          <ul>
            {Lists.find((item) => item.Type === "division").lists.map(
              (list) => {
                return (
                  <li
                    key={list.value}
