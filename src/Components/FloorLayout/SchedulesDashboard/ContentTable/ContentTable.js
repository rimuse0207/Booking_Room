        ...
        <tbody>
          {TableData.filter((item) =>
            Selected_Lists.find((i) => i.Type === "place").lists.length > 0
              ? Selected_Lists.find((i) => i.Type === "place").lists.includes(
                  item.places,
                )
              : item,
          )
            .filter((item) =>
              Selected_Lists.find((i) => i.Type === "department").lists.length >
              0
                ? Selected_Lists.find(
                    (i) => i.Type === "department",
                  ).lists.includes(item.department)
                : item,
            )
            // .filter((item) =>
            //   Selected_Lists.find((i) => i.Type === "team").lists.length > 0
            //     ? Selected_Lists.find((i) => i.Type === "team").lists.includes(
            //         item.second_department.toUpperCase(),
            //       )
            //     : item,
            // )
            .filter((item) => {
              const selectedTeam = Selected_Lists.find(
                (i) => i.Type === "team",
              );

              return selectedTeam.lists.length > 0
                ? selectedTeam.lists.some((listContent) =>
                    item.second_department
                      .toUpperCase()
                      .includes(listContent.toUpperCase()),
                  )
                : true;
            })
            .filter((item) =>
