...
const UserBookingCheck = () => {
    ...
    const [Booking_Info, setBooking_Info] = useState([]);
      const HandleClickExcelDownload = async () => {
        try {
            const Getting_Booking_Info_Data_Excel_Download_Axios = await request.get('/users/Getting_Booking_Info_Data_Excel_Download', {
                params: {
                    Month_Date
                }
            })
            if (Getting_Booking_Info_Data_Excel_Download_Axios.data.dataSuccess) {
    ...
    const Getting_Booking_Info_Data = async () => {
        try {
            ...
            const Getting_Booking_Info_Data_Axios = await request.get(`/users/Getting_Booking_Info_Data`, {
                params: {
                      Month_Date
                }
            })

            if (Getting_Booking_Info_Data_Axios.data.dataSuccess) {
                setBooking_Info(Getting_Booking_Info_Data_Axios.data.Getting_Booking_Info_Data_Rows)    
            }
