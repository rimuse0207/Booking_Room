...
import { Axios_Get_Moduls, request, Request_Get_Axios } from "../../../API";
...
const UserLoginCheck = () => {
  ...
  const HandleClickExcelDownload = async () => {
    try {
      const Login_User_Data_Checking_Excel_Download_Axios =
        await Request_Get_Axios(
          "/users/Login_User_Data_Checking_Excel_Download",
          Month_Date
        );

      if (Login_User_Data_Checking_Excel_Download_Axios.status) {
        window.open(
          `${process.env.REACT_APP_DB_HOST}/${Login_User_Data_Checking_Excel_Download_Axios.URL}`
        );
      } else
        toast.show({
          title: `IT팀에게 문의바랍니다.`,
  ...
  const Login_User_Data_Checking = async () => {
    try {
      const Login_User_Data_Checking_Axios = await Request_Get_Axios(
        "/users/Login_User_Data_Checking",
        Month_Date
      );

      if (Login_User_Data_Checking_Axios.status) {
      ...
      <div style={{ textAlign: "center", position: "relative" }}>
        <div className="Date_Show_Click_Main_Container">
          <div
            ...
            onClick={() => {
              setMonth_Date(
                moment(Month_Date).subtract(1, "months").format("YYYY-MM")
              );
            }}
          >
          ...
          <div
            ...
            onClick={() =>
              setMonth_Date(
                moment(Month_Date).add(1, "months").format("YYYY-MM")
              )
            }
          >
          ...
          {Login_Check_Data.map((list, j) => {
            return (
              <tr>
                ...
                <td>
                  {moment(list.brity_works_login_access_info_date).format(
                    "YYYY-MM-DD HH:mm"
                  )}
                </td>
              </tr>
