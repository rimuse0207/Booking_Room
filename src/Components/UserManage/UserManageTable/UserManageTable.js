...
const UserManageTable = ({ AddUserModalIsOpen, setAddUserModalIsOpen }) => {
  ...
  const getUserInfoData = async () => {
    try {
      const GetUserInfoDataFromServer = await Request_Get_Axios(
        `/users/User_Data_Getting_From_Admin`
      );

      if (GetUserInfoDataFromServer.status) {
        setUsersInfoDatas(GetUserInfoDataFromServer.data);
      } else {
        toast.show({
  ...
  const handleDeleteButton = async (UserInfoData) => {
    try {
      if (
        !window.confirm(
          `${UserInfoData.brity_works_user_info_name}님을 정말 삭제 하시겠습니까?`
        )
      ) {
        return;
      }

      const DeleteUserInfoDataFromServer = await Request_Post_Axios(
        `/users/User_Data_Delete_From_Admin`,
        {
          UserInfoData,
        }
      );

      if (DeleteUserInfoDataFromServer.status) {
        getUserInfoData();
        toast.show({
  ...
  const handleResetPassword = async (UserInfoData) => {
    try {
      if (
        !window.confirm(
          `${UserInfoData.brity_works_user_info_name}님의 비밀번호를 초기화 하시겠습니까?`
        )
      ) {
        return;
      }
      const ResetPasswordUserInfoDataFromServer = await Request_Post_Axios(
        `/users/User_Data_Reset_Password_From_Admin`,
        {
          UserInfoData,
        }
      );

      if (ResetPasswordUserInfoDataFromServer.status) {
        toast.show({
  ...
  return (
    <UserManageTableMainDivBox>
      <table className="type09">
        ...
        <tbody>
          {UsersInfoDatas.filter((list) =>
            list.brity_works_user_info_id.includes(SearchData.Email_Search)
          )
            .filter((list) =>
              list.brity_works_user_info_name.includes(SearchData.Name_Search)
            )
            .map((list, i) => {
              return (
                <tr
                  ...
                  style={
                    moment(list.brity_works_user_info_create_date).format(
                      "YYYY-MM-DD"
                ...
                >
                  ...
                  <td>
                    <div
                      ...
                      onClick={() => handleResetPassword(list)}
                    >
